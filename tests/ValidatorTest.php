...
class ValidatorTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        Validator::addExtension('channel_range', function ($field, $value, array $parameters, $validator) {
            $this->assertArrayHasKey($field, $validator->data());

            return false !== array_search($value, ['google', 'bing']);
        });

        $validator = new Validator($this->getData(), $this->getRules());

        $this->assertSame(true, $validator->validate());
        $this->assertSame($this->getData(), $validator->data());
